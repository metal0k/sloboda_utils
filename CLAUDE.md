...
**State** (`src/state.ts`): Pub/sub singleton. Single `State` object (`{ campaign, done: Set<HouseId>, issue: Set<HouseId>, updatedAt }`). Persisted to `localStorage` key `slobodaState/v2`. `subscribe(fn)` returns an unsubscribe callback; mutations (`cycleStatus`, `setStatus`, `setCampaign`, `clearAll`, `replaceState`) all call `emit()` → subscribers re-render.
...
- `src/houses.generated.ts` is also gitignored; it is regenerated on every build.
